...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await Order_itemsDBApi.findAll(req.query, { currentUser });
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'quantity', 'price'];
      const opts = { fields };
      try {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    const currentUser = req.currentUser;
    const payload = await Order_itemsDBApi.findAll(req.query, null, {
      countOnly: true,
      currentUser,
    });
